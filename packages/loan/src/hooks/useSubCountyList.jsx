...
const useSubCountyList = (
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const { data } = await axiosInstance.get(`${BASE_URL}/subcounties/paginate`, {
        params: {
  ...
  const handleDeletion = useCallback(async (item, itemName) => {
    ...
    if (isNameMatched) {
      ...
      try {
        const { data } = await axiosInstance.delete(`${BASE_URL}/subcounties/by/${item.id}`);
...
export default useSubCountyList;