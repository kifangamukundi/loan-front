...
const useOfficerList = (
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const { data } = await axiosInstance.get(`${BASE_URL}/officers/paginate`, {
        params: {
  ...
  const handleDeletion = useCallback(async (item, itemName) => {
    ...
    if (isNameMatched) {
      ...
      try {
        const { data } = await axiosInstance.delete(`${BASE_URL}/officers/by/${item.id}`);
