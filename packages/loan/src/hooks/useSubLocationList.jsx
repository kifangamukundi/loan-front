...
const useSubLocationList = (
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const { data } = await axiosInstance.get(`${BASE_URL}/sublocations/paginate`, {
        params: {
  ...
  const handleDeletion = useCallback(async (item, itemName) => {
    ...
    if (isNameMatched) {
      ...
      try {
        const { data } = await axiosInstance.delete(`${BASE_URL}/sublocations/by/${item.id}`);
        ...
        deleteDispatch({
          type: 'DELETE_FAIL',
          payload: DashboardGetError(error),
        });
      }
    }
...
export default useSubLocationList;