...
const useVillageList = (
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const { data } = await axiosInstance.get(`${BASE_URL}/villages/paginate`, {
        params: {
  ...
  const handleDeletion = useCallback(async (item, itemName) => {
    ...
    if (isNameMatched) {
      ...
      try {
        const { data } = await axiosInstance.delete(`${BASE_URL}/villages/by/${item.id}`);
