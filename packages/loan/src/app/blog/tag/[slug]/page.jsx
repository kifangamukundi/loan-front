...
import { getTag, getTags, searchPostByTag } from "@/ssr-functions";
...
export async function generateMetadata({ params }) {
  try {
    ...
    if (!tag || !tag.data || !tag.data.item) notFound();


    const { title, snippet, slug } = tag.data.item;
...
const Tag = async ({ searchParams, params }) => {
  ...
  try {
    data = await searchPostByTag({ slug, filters, sortOrder, page, limit, search });
  } catch (error) {
                                ...
                                <div className="flex flex-wrap mt-4 space-x-2">
                                    {item.categories.map((category) => (
                                        <Link href={`/blog/category/${category.slug}`} key={category.id}>
                                            <span className="bg-blue-100 text-blue-800 hover:text-gray-800 hover:bg-gray-100 transition-colors duration-200 text-md font-bold py-1 px-2 rounded-full">
                                                {category.name}
                                            </span>
                                        </Link>
                                    ))}
