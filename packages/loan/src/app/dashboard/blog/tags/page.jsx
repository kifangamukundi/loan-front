...
export default function Tags() {
    ...
    const {
        ...
        goToNextPage,
        goToLastPage,
      } = useTagList(setShowModal, setSelectedItem);
  ...
  return (
    <div className="px-4">
        <DashboardAddNewItem title='Add New Tag' link='/dashboard/blog/tags/new' loading={data.loading} />
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
            <DashbordSearchbar placeholder='Search for Tags' search={search} change={handleSearchChange} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {data?.data?.items?.map(item => (
                <DashboardGenericCard 
                    ...
                    editLink={`/dashboard/blog/tags/edit/${item.id}`}
                />
            ))}
        </div>
