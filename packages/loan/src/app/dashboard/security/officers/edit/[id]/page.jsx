...
export default function EditOfficer() {
  const { id } = useParams();
  const { data } = useResourceSingle(BASE_URL, 'officers/by', id);
  console.log(data)
  ...
  const { data: dataUpdated, updateResource } = useResourceUpdate(BASE_URL, `officers/by`, id, formData, setFormData);
  const handleSubmit = async (e) => {
    e.preventDefault();
    const updateItem = {
      IsActive: selectedIsActive,
    };
    await updateResource(updateItem, '/dashboard/security/officers');
  };
  ...
  return (
    <DashboardContainerWrapper>
      <DashboardHeading title="Edit Officer" />
      <DashboardFormWrapper>
        <DashboardInputWrapper>
          <DashboardInput
            label="First Name"
