...
export default function Groups() {
    ...
    const {
        ...
        goToNextPage,
        goToLastPage,
      } = useGroupList(setShowModal, setSelectedItem);
  ...
  return (
    <div className="px-4">
        <DashboardAddNewItem title='Add New Group' link='/dashboard/security/groups/new' loading={data.loading} />
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
            <DashbordSearchbar placeholder='Search for Groups' search={search} change={handleSearchChange} />
        ...
        {isTableView ? (
            <DashboardTable
                ...
                columns={columns}
                editLink={`/dashboard/security/groups/edit`}
            />
        ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {data?.data?.items?.map((item) => (
                    <DashboardGenericCard 
                        key={item.id} 
                        item={item}
                        editLink={`/dashboard/security/groups/edit/${item.id}`}
                    />
                ))}
            </div>
        )}
