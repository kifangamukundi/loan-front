...
export default function Roads() {
    ...
    const {
        ...
        goToNextPage,
        goToLastPage,
      } = useRoadList(setShowModal, setSelectedItem);
  ...
  return (
    <div className="px-4">
        <DashboardAddNewItem title='Add New Road' link='/dashboard/data/roads/new' loading={data.loading} />
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
            <DashbordSearchbar placeholder='Search for Roads' search={search} change={handleSearchChange} />
        ...
        {isTableView ? (
            <DashboardTable
                ...
                editLink={`/dashboard/data/roads/edit`}
            />
        ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {data?.data?.items?.map((item) => (
                    <DashboardGenericCard 
                        ...
                        item={item}
                        onDelete={handleDeleteConfirmation}
                        editLink={`/dashboard/data/roads/edit/${item.id}`}
                    />
                ))}
            </div>
        )}
