...
import { useSubLocationList } from '@/hooks';
...
export default function SubLocations() {
    ...
    const {
        ...
        goToNextPage,
        goToLastPage,
      } = useSubLocationList(setShowModal, setSelectedItem);
  ...
  return (
    <div className="px-4">
        <DashboardAddNewItem title='Add New Sub Locations' link='/dashboard/data/sublocations/new' loading={data.loading} />
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
            <DashbordSearchbar placeholder='Search for SubLocations' search={search} change={handleSearchChange} />
        ...
        {isTableView ? (
            <DashboardTable
                ...
                editLink={`/dashboard/data/sublocations/edit`}
            />
        ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {data?.data?.items?.map((item) => (
                    <DashboardGenericCard 
                        ...
                        item={item}
                        onDelete={handleDeleteConfirmation}
                        editLink={`/dashboard/data/sublocations/edit/${item.id}`}
                    />
                ))}
            </div>
        )}
