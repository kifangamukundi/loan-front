...
export default function Countries() {
    ...
    const {
        ...
        goToNextPage,
        goToLastPage,
      } = useCountryList(setShowModal, setSelectedItem);
  ...
  return (
    <div className="px-4">
        <DashboardAddNewItem title='Add New Country' link='/dashboard/data/countries/new' loading={data.loading} />
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
            <DashbordSearchbar placeholder='Search for Countries' search={search} change={handleSearchChange} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {data?.data?.items?.map(item => (
                <DashboardGenericCard 
                    ...
                    editLink={`/dashboard/data/countries/edit/${item.id}`}
                />
            ))}
        </div>
