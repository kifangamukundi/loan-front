...
import { useRegionList } from '@/hooks';
...
export default function Regions() {
    ...
    const {
        ...
        goToNextPage,
        goToLastPage,
      } = useRegionList(setShowModal, setSelectedItem);
  ...
  return (
    <div className="px-4">
        <DashboardAddNewItem title='Add New Region' link='/dashboard/data/regions/new' loading={data.loading} />
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
            <DashbordSearchbar placeholder='Search for Regions' search={search} change={handleSearchChange} />
        ...
        {isTableView ? (
            <DashboardTable
                ...
                editLink={`/dashboard/data/regions/edit`}
            />
        ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {data?.data?.items?.map((item) => (
                    <DashboardGenericCard 
                        ...
                        item={item}
                        onDelete={handleDeleteConfirmation}
                        editLink={`/dashboard/data/regions/edit/${item.id}`}
                    />
                ))}
            </div>
        )}
