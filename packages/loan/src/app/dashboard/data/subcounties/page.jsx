...
import { useSubCountyList } from '@/hooks';
...
export default function SubCounties() {
    ...
    const {
        ...
        goToNextPage,
        goToLastPage,
      } = useSubCountyList(setShowModal, setSelectedItem);
  ...
  return (
    <div className="px-4">
        <DashboardAddNewItem title='Add New Sub County' link='/dashboard/data/subcounties/new' loading={data.loading} />
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
            <DashbordSearchbar placeholder='Search for SubCounties' search={search} change={handleSearchChange} />
        ...
        {isTableView ? (
            <DashboardTable
                ...
                editLink={`/dashboard/data/subcounties/edit`}
            />
        ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {data?.data?.items?.map((item) => (
                    <DashboardGenericCard 
                        ...
                        item={item}
                        onDelete={handleDeleteConfirmation}
                        editLink={`/dashboard/data/subcounties/edit/${item.id}`}
                    />
                ))}
            </div>
        )}
