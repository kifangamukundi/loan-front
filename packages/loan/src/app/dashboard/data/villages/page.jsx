...
import { useVillageList } from '@/hooks';
...
export default function Villages() {
    ...
    const {
        ...
        goToNextPage,
        goToLastPage,
      } = useVillageList(setShowModal, setSelectedItem);
  ...
  return (
    <div className="px-4">
        <DashboardAddNewItem title='Add New Village' link='/dashboard/data/villages/new' loading={data.loading} />
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
            <DashbordSearchbar placeholder='Search for Villages' search={search} change={handleSearchChange} />
        ...
        {isTableView ? (
            <DashboardTable
                ...
                editLink={`/dashboard/data/villages/edit`}
            />
        ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {data?.data?.items?.map((item) => (
                    <DashboardGenericCard 
                        ...
                        item={item}
                        onDelete={handleDeleteConfirmation}
                        editLink={`/dashboard/data/villages/edit/${item.id}`}
                    />
                ))}
            </div>
        )}
