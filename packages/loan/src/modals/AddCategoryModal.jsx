...
export default function AddCategoryModal({ showAddModal, createFormData, handleCreate, handleAddCancel, handleChange, createData }) {
      ...
      <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className={`inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
          ...
          <div className={`px-4 py-3 sm:px-6 flex justify-end space-x-4 ${createData.loading ? 'animate-pulse' : ''}`}>
            ...
            <button
              onClick={() => handleCreate({ CategoryName: createFormData.categoryname, Posts: [] })}
              className={`inline-flex justify-center px-4 py-2 bg-green-600 text-white font-medium rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 ${createData.loading ? 'opacity-50 cursor-not-allowed' : ''}`}
              disabled={createData.loading}
            >
              {createData.loading ? (
